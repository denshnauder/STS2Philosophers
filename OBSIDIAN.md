...
本项目的开发笔记以 Obsidian 仓库中的版本为唯一真源：

- Vault：`STS2mod`
- 目录：`STS2mod` Vault 内的 `STS2 Mod 开发`
- 本机真源绝对路径：`G:\我的云端硬盘\STS2mod\STS2 Mod 开发`。开始任务时直接读取这里的活动问题，不只根据同名 Vault 推断路径。
- `S:\01_Workspace\Obsidian\STS2mod` 已于2026年9月17日按用户要求同步项目笔记，作为阅读副本；不在该副本独立维护问题状态，不以它恢复开发真源。同步采用G盘到S盘单向复制，覆盖前备份并保留S盘独有文件；2026年9月17日更新定时接续后，每个实质阶段单向同步内容变化的受影响项目笔记；没有变化不复制。不可把旧副本覆盖回G盘。
...
Obsidian 的 `08问题闭环/08问题与修改.md` 是用户查看活动问题、阶段结果和下一步的主要入口。仓库 `docs/issues/ACTIVE.md` 仅作为 Codex 跨窗口恢复上下文的轻量镜像，不能替代 Obsidian 更新。

代码和构建相关文档保存在本仓库；开发日志、活动问题、研究资料、设计灵感与读书式笔记保存在上述 Obsidian 目录。
