...
(async()=>{
  ...
  try{
    ...
    assert.match(await page.locator('#invitation').innerText(),/普罗泰戈拉.*共同处理事务/);assert.match(await page.locator('main').innerText(),/不是原典引文/);
    ...
    await reset();await b('对照：各用自己的份数').click();assert.match(await page.locator('#status').innerText(),/生命8\/10/);assert.match(await page.locator('#records').innerText(),/没有教学指令/);assert.match(await page.locator('#question').innerText(),/没有指定后续操作指令/);assert.match(await page.locator('#question').innerText(),/仍可能有教育/);assert.match(await page.locator('#questionAttribution').innerText(),/普罗泰戈拉.*游戏对话/);
    await reset();await b('不参与，直接离开').click();assert.match(await page.locator('#ending').innerText(),/没有进入这次补给实践.*生命仍6/);assert.match(await page.locator('#ending').innerText(),/日常教育效果未观测/);assert.equal(await b('离开这次共同实践').isVisible(),false);
