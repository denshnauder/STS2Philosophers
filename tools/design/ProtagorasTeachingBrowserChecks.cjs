...
(async()=>{
  ...
  try{
    ...
    await reset();await b('这次采用：每次给生命最低者').click();await b('记住本次份数').click();await b('亲自改分：按贡献份数').click();assert.match(await page.locator('#records').innerText(),/休整实际回复0，玩家最终6/);assert.match(await page.locator('#question').innerText(),/你自己的分配/);assert.equal(await b('交给他按原指令执行').isVisible(),false);
    ...
    await reset();await b('这次采用：按贡献份数').click();await b('记住本次份数').click();await b('结束共用，各用本轮自己的份数').click();assert.match(await page.locator('#status').innerText(),/生命8\/10/);assert.match(await page.locator('#records').innerText(),/结束共用，各自使用/);assert.match(await page.locator('#question').innerText(),/不等于没人能学会/);assert.equal(await b('这次采用他的替案，由他执行').isVisible(),false);
    ...
    assert.equal(errors.length,0,errors.join('\n'));console.log('PASS instruction/refusal/counterproposal/control/end-shared/self-use/exit, safe text and 860/360/320 light/dark browser checks; no native game or fun verdict.');
