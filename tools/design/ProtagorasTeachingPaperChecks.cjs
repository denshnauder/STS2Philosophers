...
check('Equal shares have identical material with distinct explicit instructions',()=>{
  const a=open('equal','sample'),b=open('equal','method');assert.equal(json(p.preview(a)),json(p.preview(b)));assert.notEqual(json(a.teaching),json(b.teaching));p.execute(a);p.execute(b);assert.equal(a.party[0].hp,b.party[0].hp);assert.notEqual(p.question(a),p.question(b));
});
check('Rotating contributors changes method, not a remembered seat example',()=>{
  const a=open('contribution','sample','rotation'),b=open('contribution','method','rotation');assert.equal(json(p.preview(a).allocation),json([2,0,1]));assert.equal(json(p.preview(b).allocation),json([0,2,1]));assert.equal(p.preview(a).finalHp,10);assert.equal(p.preview(b).finalHp,8);
});
check('Self use compares identical inputs, travel and rest without teaching or moral rewards',()=>{
  for(const fixture of ['newcomer','rotation']){const a=p.create(fixture),b=open('contribution','method',fixture);assert.equal(p.baseline(a),true);p.execute(b);assert.equal(json(a.party),json(b.party));assert.equal(a.teaching,null);assert.equal(json(a.travel),json(b.travel));assert.match(p.question(a),/没有教学材料/);assert.equal('virtueScore' in a,false);assert.equal('learnedVirtue' in a,false);}
});
...
check('Premature, invalid and repeated confirmations leave all records unchanged',()=>{
  const s=p.create();unchanged(s,()=>p.teach(s,'method'));unchanged(s,()=>p.execute(s));unchanged(s,()=>p.finish(s));unchanged(s,()=>p.first(s,'__proto__'));p.first(s,'equal');unchanged(s,()=>p.first(s,'lowest'));unchanged(s,()=>p.baseline(s));unchanged(s,()=>p.skip(s));unchanged(s,()=>p.teach(s,'other'));p.teach(s,'method');unchanged(s,()=>p.teach(s,'sample'));unchanged(s,()=>p.execute(s,'other'));p.execute(s);unchanged(s,()=>p.execute(s));unchanged(s,()=>p.finish(s,42));unchanged(s,()=>p.finish(s,'x'.repeat(301)));p.finish(s);unchanged(s,()=>p.finish(s,'改写已结束的记录'));
});
...
check('All fixture/instruction/control paths spend six supplies once and retain valid life',()=>{
  for(const fixture of ['newcomer','rotation'])for(const key of Object.keys(p.rules))for(const kind of ['sample','method'])for(const override of [null,...Object.keys(p.rules)]){
    const s=open(key,kind,fixture);assert.equal(p.execute(s,override),true);assert.equal(s.records.reduce((n,r)=>n+r.allocation.reduce((m,x)=>m+x,0),0),6);assert.equal(s.records.length,2);assert.ok(s.party.every(x=>x.hp>0&&x.hp<=10));unchanged(s,()=>p.execute(s,override));assert.equal(p.finish(s),true);
  }
});
